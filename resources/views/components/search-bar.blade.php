<form action="{{ route('search.posts') }}" id="search_form" method="GET">
    <div class="p-1 w-full rounded rounded-pill shadow-sm position-relative {{ $theme == 'dark' ? 'bg-gray-900 text-gray-100' : 'bg-gray-300 text-gray-800' }}">
        <div class="input-group">
            ...
            <input type="search" placeholder="Search" name="search" aria-describedby="button-addon2" id="search" class="form-control border-0 rounded rounded-pill {{ $theme == 'dark' ? 'bg-gray-600 text-gray-100' : 'bg-gray-100 text-gray-800' }}"/>
